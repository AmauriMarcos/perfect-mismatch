...
const Navbar = () => {
  ...
  return (
    <div
      className={`w-full flex flex-col ${stickyClass === "sticky" && "fixed"}`}
    >
      <div
        className={` ${
          stickyClass === "sticky" ? "hidden" : "flex"
        } w-full items-center justify-center py-[10px] px-[15px] gap-2 h-[45px] bg-tertiary hover:bg-secondary group cursor-pointer`}
      >
        <IconSearch className="h-4 w-4 md:h-6 md:w-6 fill-secondary-foreground cursor-pointer group-hover:fill-background" />
        <h4 className="font-montserrat text-[.525rem] md:text-[.65rem] uppercase font-extrabold text-secondary-foreground cursor-pointer group-hover:text-background">
      ...
      <div className="w-full px-[6%] md:px-[12%] 2xl:px-[14%] h-full bg-background flex flex-col justify-between items-center shadow-md ">
        <div
          className={`hidden ${
            stickyClass === "sticky" ? "hidden" : "md:flex"
          } justify-between h-[56px] w-full py-[1.2rem]`}
        >
          <ul className="hidden md:flex gap-4 transition-all duration-[350ms] ease-in-out ">
            {topLinks.map((item) => (
              <li key={item.id} className="hover:text-slate-600">
                <Link
                  href={item.path}
                  className={`hover:text-orange-300 block transition-all duration-[350ms] ease-in-out text-[.875rem] ${
                    item.path === pathname
                      ? "text-foreground"
        ...
        >
          ...
          <ul className="hidden md:flex items-center relative gap-6 grow m-auto justify-end transition-all duration-[350ms] ease-in-out">
            {items.map((item) => (
            ...
            {stickyClass === "sticky" && (
             ...
             {stickyClass === "sticky" && (
              <button className="bg-transparent text-[1.2rem] font-extrabold border-2 border-secondary rounded-[25px] hover:text-secondary px-[1rem] py-[.2em]">
                Login
              </button>
            )}
          </ul>
